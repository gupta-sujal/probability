...
% \end{description}
\\
The equaion of the incircle is given by
...
\\
Let us define $\vec{n} = \vec{C}-\vec{B}$ and $\vec{I}$ is the incentre of the tri.ABC\\
\\
...
Substituting the value of $ \vec{x} $ from (1) in (2)
\begin{align*}
  &\implies\norm{ \vec{B} + k(\vec{C}-\vec{B})- \vec{I} }^2 = r^2 \\
  &\implies(\vec{B} + k(\vec{C}-\vec{B})- \vec{I})  \cdot (\vec{B} + k(\vec{C}-\vec{B})- \vec{I}) = r^2 
\end{align*}
On simplifying the above equation:
\begin{align*}
k^2\norm{n}^2 +2k\vec{n}\cdot({\vec{B}-\vec{I}})+\norm{I}^2 +\norm{B}^2-2(\vec{B}\cdot\vec{I}) - r^2&=0
\end{align*}
The above is a quadratic equation in k.
\\\\
The Discriminant of the quadratic equation is:\\
${(2k\vec{n}\cdot({\vec{B}-\vec{I}}))}^2-4(\norm{n}^2)(\norm{I}^2 +\norm{B}^2-2\vec{B}\cdot\vec{I} - r^2)$
\\
...
And the solution is $$k=\frac{-(2\vec{n}\cdot({\vec{B}-\vec{I}}))}{2{\ \norm{n}^2}}$$\\
On substituting the values, the value of k is 
$$k=\frac{122\sqrt{74}+82\sqrt{122}}{122(\sqrt{74}+\sqrt{122}+\sqrt{32})}$$
%%%%%%%%%%

So on substituting this value in (1), we get the point $\vec{D_{3}}$ \\
$\vec{D_{3}} = \frac{1}{\sqrt{74}+\sqrt{32}+\sqrt{122}}\myvec{{-366\sqrt{74}-406\sqrt{122}-488\sqrt{32}}\\{-610\sqrt{74}+732\sqrt{32}-170\sqrt{122}}}$
